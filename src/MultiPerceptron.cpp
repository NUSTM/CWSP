...
namespace cwsp
{
    ...
    {
        cout << "MultiPerceptron loading model..." << endl;
        omega.clear();
        ...
        std::cout << "Weight matrix size: "<<class_set_size<<"*"<<feat_set_size<<endl;
        return true;
    }

    bool MultiPerceptron::load_bin_model(string model_file)
    {
        cout << "MultiPerceptron loading binary model..." << endl;
        omega.clear();
    ...
    bool MultiPerceptron::read_samps_vec(vector<vector<int> >featsVec, vector<feature> &samp_feat_vec)
    {
        if (featsVec.size()==0)
        {
            cerr << "\nMultiPerceptron ERROR"<<endl;
        ...
        for(size_t i=0;i<featsVec.size();++i)
        {
            ...
            {
                samp_feat.id_vec.push_back(*it);
                samp_feat.value_vec.push_back(1.0);
            }
            samp_feat_vec.push_back(samp_feat);
        }
        return true;
    ...
    bool MultiPerceptron::load_training_data(int feat_size, int class_size, vector<feature> feat_vec, vector<int> class_vec)
    {
        cout<< "Loading training data..." <<endl;
        samp_feat_vec = feat_vec;
        samp_class_vec = class_vec;
        if( samp_feat_vec.size()==0 || samp_class_vec.size()==0) return false;
        feat_set_size = feat_size;
        class_set_size = class_size;
        std::cout<<"Class size:" << class_set_size<<endl;
        std::cout<<"Feature size:"<<feat_set_size<<endl;
        return true;
    }
    ...
    bool MultiPerceptron::classify_samps_withprb(vector<vector<int> >featsVec, vector<vector<double> > &probs)
    {
        vector<feature> test_feat_vec;
